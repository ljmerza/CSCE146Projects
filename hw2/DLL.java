...
**/
  public boolean add(T oneLine)
  {
  	this.sortAdd(oneLine);
    return true;
...
/*********************************************************************
 * This method uses an insert sort to search through each node's 
 * candidate name and insert the new node into alphabetic order by
 * the candidate name.
 * 
 * @param the <code>OneLine</code> data that will
 * be used in inserting in the <code>DLL</code> by insert sort.
**/
  private void sortAdd(T oneLine)
  {

  	if( this.getSize() > 2 )
  	{
    	DLLNode<T> newNode = new DLLNode<T>();
    	DLLNode<T> searchNode = new DLLNode<T>();
    	
    	newNode.setNodeData(oneLine);
    	searchNode = this.getHead();
    	searchNode = searchNode.getNext();
    	
  		for( int i = 2; (i < this.getSize()) && (newNode.getNodeData().compareTo(searchNode.getNodeData()) == 1); i++)
    	{
  			searchNode = searchNode.getNext();
    	}  // while loop

    	searchNode = searchNode.getPrev();
    	this.linkAfter(searchNode, newNode);
  	} // if statement
  	else
  	{
  		this.addAtHead(oneLine);
  	} // else statement
  } // private void sortAdd(T oneLine)
